...
for d in "${dirs[@]}"; do
  ...
  raw_n="$(find "$d/00_input/raw" -type f ! -name .gitkeep 2>/dev/null | wc -l)"
  idx_n="$(grep -c '^| [0-9]' "$d/00_input/INDEX.md" 2>/dev/null)"; idx_n="${idx_n:-0}"
  open_ai="$(grep -E '^\| A-[0-9]+' "$d/04_schedule/ACTION_ITEMS.md" 2>/dev/null | grep -vc '완료')"; open_ai="${open_ai:-0}"
