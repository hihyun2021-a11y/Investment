...
3. **자료 접수**: `projects/<코드명>/00_원본자료/raw/`에 새 파일이 보이면 항상 data-steward를 먼저 실행한다. data-steward는 `platform/scripts/extract_text.py`로 `00_원본자료/text/`에 텍스트 추출본을 만들고, 다른 에이전트는 원본 대신 이 추출본을 Read로 읽는다 (인용은 원본 파일명·페이지로). 다른 에이전트는 data-steward가 만든 `00_원본자료/processed/` 값만 인용한다.
